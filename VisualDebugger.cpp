...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 100.0f, 175.0f), PxVec3(0.f,-1.f,-1.f), 5.f);
	...
	void HUDInit()
	{
		...
		//add a help screen
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " SHA12307610 James Shapcott - CGP3001M Game Engines - Assesment Item 1");
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Score: " + std::to_string(scene->scoreUpdate));
		hud.AddLine(HELP, " Lives: " + std::to_string(scene->livesUpdate));
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Use E and R to activate the flippers...");
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Press Space to pull the plunger for the ball...");
		hud.AddLine(HELP, "");
	...
	void UserKeyPress(int key)
	{
		switch (toupper(key))
		{
		case ' ': //Move Plunger up
			scene->plunger->Get()->isRigidDynamic()->addForce(PxVec3(0.0f, 0.0f, 1000.0f));
			break;
		case 'E': //left flipper lower
			scene->flipperJoint_Left->DriveVelocity(5.0f);
			scene->flipperJoint_Left_2->DriveVelocity(5.0f);
			break;
		case 'R': //right flipper lower
			scene->flipperJoint_Right->DriveVelocity(-5.0f);
			scene->flipperJoint_Right_2->DriveVelocity(-5.0f);
			break;
		default:
			break;
		}
	...
	void UserKeyRelease(int key)
	{
		...
		{
		case ' ': //Reset Plunger
			scene->plunger->Get()->isRigidDynamic()->addForce(PxVec3(0.0f, 0.0f, -5000.0f));
			break;
		case 'E': //left flipper lower
			scene->flipperJoint_Left->DriveVelocity(-5.0f);
			scene->flipperJoint_Left_2->DriveVelocity(-5.0f);
